...
void    handleConsole(int argc, char **argv){
    ...
    for (int i = 2; i < argc-1; i += 2){
        argStr = argv[i];
        argMod = stoi(argv[i+1]);
        if(argStr == "bonusUp") editBonusCP(argMod);
        if(argStr == "bonusDn") editBonusCP(argMod, true);
        if(argStr == "levelUp") editLevel(argMod);
        if(argStr == "levelDn") editLevel(argMod, true);
    }
    fileIO.saveFile();
}

void    editBonusCP(int modNum, bool flagDown){
    int bonusMod = modNum * pow(-1, flagDown);
    culInf.setBonusCP(culInf.get_Bonus_CP() + bonusMod);
    cout << "Bonus CP is now: " << culInf.get_Bonus_CP() << endl;
}

void    editLevel(int modNum, bool flagDown){
    int levelMod = modNum * pow(-1, flagDown);
    culInf.setLevel(culInf.get_Level() + levelMod);
    cout << "Culture Level is now: " << culInf.get_Level() << endl;
}
